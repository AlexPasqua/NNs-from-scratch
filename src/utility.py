...
def plot_curves(tr_loss, val_loss, tr_acc, val_acc, path=None, lr=None, momentum=None, lambd=None, **kwargs):
    ...
    ax[0].plot(range(len(val_loss)), val_loss, color='r', label='metric error')
    ...
    ax[0].grid()
    ax[0].set_ylim((0., 10.))
    ax[1].plot(range(len(tr_acc)), tr_acc, color='b', linestyle='dashed', label='development metric')
    ax[1].plot(range(len(val_acc)), val_acc, color='r', label='internal metric')
